...
export function getProviderDisplayName(providerId?: string, locale?: string): string {
  ...
  const provider = PRESET_PROVIDERS.find((item) => item.id === providerKey)
...
export const PROVIDER_TUTORIALS: ProviderTutorial[] = [
  ...
  {
    providerId: 'google',
    steps: [{ text: 'google_step1', url: 'https://aistudio.google.com/api-keys' }],
  },
  {
    providerId: 'minimax',
    steps: [{ text: 'minimax_step1', url: 'https://platform.minimaxi.com/user-center/basic-information/interface-key' }],
  },
  {
    providerId: 'vidu',
    steps: [{ text: 'vidu_step1', url: 'https://platform.vidu.cn/api-keys' }],
  },
  {
    providerId: 'bailian',
    steps: [{ text: 'bailian_step1', url: 'https://bailian.console.aliyun.com/cn-beijing/?tab=model#/api-key' }],
  },
]

export function getProviderTutorial(providerId: string): ProviderTutorial | undefined {
  const providerKey = getProviderKey(providerId)
  return PROVIDER_TUTORIALS.find((tutorial) => tutorial.providerId === providerKey)
}
